<?php 
	...
	if(isset($_SESSION['id'])){
    	...
    	if(empty($key_uniq)){
    		...
    		$sql = "UPDATE `summary_juni` SET `key_uniq` = '$key_uniq', `material` = '$material', `material_desc` = '$material_desc', `plant` = '$plant', `sloc` = '$sloc', `sloc_desc` = '$sloc_desc', `month` = '$month' , `val_stock_val` = '$val_stock_val', `sat_val_stock` = '$sat_val_stock', `val_stock` = '$val_stock', `satuan` = '$satuan' WHERE `summary_juni`.`id` = '$id' ";
				mysqli_query($koneksi,$sql);
				$edit++;
				// header("Location:index.php?include=header_opb&notif=editberhasil");
				header("location:index.php?include=slow_moving_juni&edit=$edit");
	  }
	}
?>